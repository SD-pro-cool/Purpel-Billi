...
class Steal(commands.Cog):
    ...
    @commands.command(name="steal", help="Steal an emoji or sticker", usage="steal <emoji>", aliases=["eadd"], with_app_command=True)
    ...
    async def steal(self, ctx, emote=None):
        ...
        if emote:
            ...
            await ctx.send(embed=discord.Embed(title="Steal", description="No emoji or sticker found", color=0x4D3164))
    ...
    async def process_emoji(self, ctx, emote):
        try:
            if emote[0] == '<':
                ...
                await ctx.send(embed=discord.Embed(title="Steal", description="Invalid emoji", color=0x4D3164))
        except Exception as e:
            await ctx.send(embed=discord.Embed(title="Steal", description=f"Failed to add emoji: {str(e)}", color=0x4D3164))

    async def add_emoji(self, ctx, url, name, animated):
        try:
            if not self.has_emoji_slot(ctx.guild, animated):
                await ctx.send(embed=discord.Embed(title="Steal", description="No more emoji slots available", color=0x4D3164))
                return
            ...
            await ctx.send(embed=discord.Embed(title="Steal", description=f"Added emoji \"**{emote}**\"!", color=0x4D3164))
        except Exception as e:
            await ctx.send(embed=discord.Embed(title="Steal", description=f"Failed to add emoji: {str(e)}", color=0x4D3164))
    ...
    async def add_sticker(self, ctx, url, name):
        try:
            if len(ctx.guild.stickers) >= self.get_max_sticker_count(ctx.guild):
                await ctx.send(embed=discord.Embed(title="Steal", description="No more sticker slots available", color=0x4D3164))
                return
            ...
            await ctx.send(embed=discord.Embed(title="Steal", description=f"Added sticker \"**{sanitized_name}**\"!", color=0x4D3164))
        except Exception as e:
            await ctx.send(embed=discord.Embed(title="Steal", description=f"Failed to add sticker: {str(e)}", color=0x4D3164))
    ...
    async def create_buttons(self, ctx, attachments, stickers, emojis):
            ...
            async def steal_as_emoji(self, interaction: discord.Interaction, button: discord.ui.Button):
                ...
                for sticker in self.stickers:
                    ...
                    if sticker.format in [discord.StickerFormatType.png, discord.StickerFormatType.apng, discord.StickerFormatType.lottie]:
                        ...
                        await self.ctx.send(embed=discord.Embed(title="Steal", description=f"Unsupported sticker format for {sticker.name}", color=0x4D3164))
        ...
        embed = discord.Embed(description="Choose what to steal:", color=0x4D3164)
        if attachments:
