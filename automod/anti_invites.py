...
from emojis import CHECK as CHECK_EMOJI
        ...
        if self.invite_pattern.search(message.content):
            ...
            try:
                ...
                try:
                    ...
                    simple_embed.description = f"{CHECK_EMOJI} | {user.mention} has been successfully **{action_taken}** for **posting an invite link.**"
                    simple_embed.set_thumbnail(url=self.bot.user.avatar.url)
                    simple_embed.set_footer(text="Powered By Meekly AutoModeration.", icon_url=self.bot.user.avatar.url)
